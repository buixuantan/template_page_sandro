...
url=this.removeQueryStringParameter('ajax',url);url=this.removeQueryStringParameter('_',url);if(url!==window.location.href){history.pushState({url:url},null,url);}},getParams:function(urlParams,paramName,paramValue,defaultValue){var decode=window.decodeURIComponent,paramData={},parameters,i;for(i=0;i<urlParams.length;i++){parameters=urlParams[i].split('=');paramData[decode(parameters[0])]=parameters[1]!==undefined?decode(parameters[1].replace(/\+/g,'%20')):'';}
...
$('body').trigger('contentUpdated');var a=setInterval(function(){if($('.ias-load-more.filter-navigation').length!==0){$('.ias-load-more').html('<span class="loadmore_progress" style="width:'+(window.countItem/window.countItems).toFixed(2)*100+'%;"></span>'+'<span class="loadmore_label">'+window.textNext+' ('+window.countItem+'/'+window.countItems+')'+'</span>');clearInterval(a);}},300);},lnSlideDown:function(){var productWrapperMarginTop=$('body').attr('data-pwmt');$('.products.wrapper').animate({marginTop:productWrapperMarginTop},{duration:500,complete:function(){$('.wp-filters span.wp-slide-down-add').addClass('active');}});$('.wp-slide-down-add:not(active)').off('click');$('.slide-down-filter').slideDown(500);$('.wp-filters').on('click',this.lnSlideUp);},lnSlideUp:function(){$('.wp-filters').off('click');$('.slide-down-filter').slideUp({duration:500,start:function(){$('.products.wrapper').animate({marginTop:'0px'},{duration:500,complete:function(){$('.wp-filters span.wp-slide-down-add').removeClass('active');$('.wp-filters').on('click',this.lnSlideDown);},queue:false});},queue:false});},reinitializeIas:function(){if(require.defined('ias')&&window.ajaxCatalog=='infiniteScroll'){jQuery.ias().destroy();jQuery(function($){var config={container:'.products.wrapper .product-items',item:'.product-item',pagination:'.toolbar .pages, .toolbar .limiter',next:'.pages .action.next',negativeMargin:window.negativeMargin};$.ajaxSetup({cache:true});$(config.container).closest('.column.main').addClass('infinite-scroll');var ias=$.ias(config);ias.getNextUrl=function(container){if(!container){container=ias.$container;}
...
return nexturl;};ias.extension(new IASPagingExtension());ias.on('pageChange',function(pageNum,scrollOffset,url){window.page=pageNum;});ias.on('load',function(event){var url=event.url;event.ajaxOptions.cache=true;event.url=window.ajaxInfiniteScroll.removeQueryStringParameter('_',event.url);});ias.on('loaded',function(data,items){window.ajaxInfiniteScroll.reloadImages(items);window.ajaxInfiniteScroll.dataLayerUpdate(data);window.countItem+=items.length;var a=setInterval(function(){if($('.ias-load-more.filter-navigation').length!==0){$('.ias-load-more').html('<span class="loadmore_progress" style="width:'+(window.countItem/window.countItems).toFixed(2)*100+'%;"></span>'+'<span class="loadmore_label">'+window.textNext+' ('+window.countItem+'/'+window.countItems+')'+'</span>');clearInterval(a);}},300);});ias.on('rendered',function(items){window.ajaxInfiniteScroll.fixAddToCart();window.ajaxInfiniteScroll.reloadQuickView();window.ajaxInfiniteScroll.reloadCategoryPage();if(window.showCanonical==1){window.ajaxInfiniteScroll.reloadCanonicalPrevNext();}
...
$("img.lazy").each(function(){$(this).css({'max-width':'100%'});});self.reinitializeIas();$('.product.photo.product-item-photo .infinitescroll').on('click',function(e){e.preventDefault();var page=$(this).attr('data-item-page');var url=window.location.href;self.resetIasPagination(page,url);var href=$(this).find('.infinitescroll-product-link').first().attr('href');window.location.href=href;});$('.product-item-info a').each(function(){if(typeof $(this).attr('data-item-page')==='undefined'){$(this).attr('data-item-page',window.page);}});if(require.defined('ias')&&window.ajaxCatalog=='nextPage'){window.ajaxInfiniteScroll.addPageSelector('.pages li.item a.page');}
...
return params;}});window.addEventListener('popstate',event=>{let state=event.state;if(state===null){location.reload();}
$('a[href=\''+state.url+'\'].page').trigger('click');});return $.mage.productListToolbarForm;});